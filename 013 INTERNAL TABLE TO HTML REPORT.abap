...
*  &---------------------------------------------------------------------*
  ...
  INCLUDE zcl_itab_to_html.
  
  data: it_itab type table of makt.
  
  START-OF-SELECTION.
  
  select * from makt into table it_itab up to 10 rows.
  
  DATA(io_converter) = NEW zcl_itab_to_html(
    " Configuração de CSS para a tabela inteira
    table_style = 'width: 80%; border: #999 1px solid; border-collapse: collapse; text-align: center;'
    " Configuração de CSS para o cabeçalho e linhas de dados
    th_style = 'font-weight: bold; border: #999 1px solid; background: #eee;'
    td_style = 'border: #999 1px solid;'
  ).
  
  " Converter a tabela interna para HTML
